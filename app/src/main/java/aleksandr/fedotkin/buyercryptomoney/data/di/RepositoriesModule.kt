...
val repositoriesModule = module {
    ...
    factory<BuyRepository> {
        BuyRepositoryImpl(networkAPI = get(), buyMapper = get())
    }

    factory<BuyerRepository> {
        BuyerRepositoryImpl(buyerMapper = get(), networkAPI = get())
    }

    factory<SellerRepository> {
        SellerRepositoryImpl(sellerMapper = get(), networkAPI = get())
    }

    factory<ProductRepository> {
        ProductRepositoryImpl(productMapper = get(), sellerRepository = get(), networkAPI = get())
    }
}
